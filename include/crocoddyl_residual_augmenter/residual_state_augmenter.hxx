...
void CostModelResidualAugmenterTpl<Scalar>::calc(
  ...
  const std::size_t nx = wrapped_model_->get_state()->get_nx();

  wrapped_model_->calc(d->wrapped_data, x.head(nx), u.head(nu));

  d->cost = d->wrapped_data->cost;
}
...
void CostModelResidualAugmenterTpl<Scalar>::calcDiff(
  ...
  d->Lu.head(nu) = d->wrapped_data->Lu;
  d->Luu.topLeftCorner(nu, nu) = d->wrapped_data->Luu;
  d->Lx.head(nx) = d->wrapped_data->Lx;
  d->Lxu.topLeftCorner(nx, nu) = d->wrapped_data->Lxu;
  d->Lxx.topLeftCorner(nu, nu) = d->wrapped_data->Lxx;
}
